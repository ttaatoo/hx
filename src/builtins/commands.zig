...
pub const top_level_specs = [_]TopLevelSpec{
    ...
    .{
        .kind = .login,
        .token = "login",
        .usage = "login [grok|codex]",
        .summary = "Sign in with SuperGrok or Codex",
    },
    .{
        .kind = .logout,
        .token = "logout",
        .usage = "logout [grok|codex]",
    ...
    .{
        ...
        .details = &.{
            ...
            "  yolo   Disable hx permissions and sandboxing",
            "",
    ...
    .{
        .kind = .provider,
        .token = "provider",
        .usage = "provider <xai|anthropic|codex>",
        .summary = "Choose SuperGrok, Anthropic, or Codex",
    },
    .{
        .kind = .doctor,
        .token = "doctor",
    ...
    .{
        ...
        .summary = "Show local hx token usage and spend",
        .options = &.{
            .{ .flag = "--period <24h|7d|30d>", .description = "Select a rolling window (default: 30d)" },
            json_option,
        },
        .details = &.{
            "Reports only usage recorded by hx on this machine.",
            "This command reads local state and does not query a remote billing API.",
        },
    },
    .{
        .kind = .upgrade,
        ...
        .summary = "Upgrade hx on the selected release channel",
        .options = &.{
...
pub const top_level_flags = [_]TopLevelFlag{
    ...
    .{
        .usage = "-v, --version",
        .description = "Print the hx version and exit",
    },
};

pub const top_level_examples = [_]TopLevelExample{
    .{ .command = "hx", .description = "Start a fresh interactive session" },
    .{ .command = "hx ask \"Explain the changes in this repository\"", .description = "Run one request and exit" },
    .{ .command = "hx session resume last", .description = "Continue the latest session for this workspace" },
    .{ .command = "hx status --json", .description = "Inspect the current configuration as JSON" },
};

pub const top_level_notes = [_][]const u8{
    "Run `hx <command> --help` for command-specific options and examples.",
    "Run `/help` inside an interactive session for slash commands.",
};

pub const top_level_resources = [_]TopLevelResource{
    .{ .label = "Learn more about hx:", .value = "https://github.com/ttaatoo/hx", .link = true },
    .{ .label = "Report a problem:", .value = "run `/feedback` inside hx" },
};
...
pub const top_level_registry = TopLevelRegistry{
    ...
    .interactive_hint = "hx starts an interactive session by default. Use `hx ask` to run one noninteractive request.",
...
pub const slash_specs = [_]SlashSpec{
    ...
    .{ .kind = .usage, .command = "/usage", .aliases = &.{"/cost"}, .help_entry = "/usage (/cost)", .completion_description = "show local hx tokens, models, and spend", .presentation_category = .account },
    ...
    .{ .kind = .provider, .command = "/provider", .help_entry = "/provider [xai|anthropic|codex]", .completion_description = "choose SuperGrok, Anthropic, or Codex", .presentation_category = .model, .has_args = true, .accepts_payload = true },
    .{ .kind = .permissions, .command = "/permissions", .help_entry = "/permissions [ask|auto|yolo|reset]", .completion_description = "choose what hx is allowed to do", .presentation_category = .security, .show_in_welcome = true, .has_args = true, .accepts_payload = true },
    ...
    .{ .kind = .feedback, .command = "/feedback", .help_entry = "/feedback", .completion_description = "open the hx feedback form", .presentation_category = .product, .show_in_welcome = true },
    ...
    .{ .kind = .version, .command = "/version", .help_entry = "/version", .completion_description = "show the hx version", .presentation_category = .general },
