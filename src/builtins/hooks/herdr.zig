...
pub const Client = struct {
    ...
    /// Report hx's session id so herdr can associate the pane with a resumable
    ...
    /// Name the pane/agent so herdr lists hx even while idle. Best-effort;
    ...
    /// Remove hx from the pane on exit: drop the reported agent and clear the
    /// label/name. Without this the pane keeps showing "hx" after hx is gone.
...
test "report_agent serializes a single newline-delimited json line" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"7\",\"method\":\"pane.report_agent\",\"params\":{\"pane_id\":\"w1:p1\"," ++
            "\"source\":\"custom:fx\",\"agent\":\"hx\",\"state\":\"working\"," ++
...
test "report_agent omits custom_status when null" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"1\",\"method\":\"pane.report_agent\",\"params\":{\"pane_id\":\"w1:p1\"," ++
            "\"source\":\"custom:fx\",\"agent\":\"hx\",\"state\":\"idle\"}}\n",
        out.written(),
    );
}
...
test "report_agent escapes pane id" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"2\",\"method\":\"pane.report_agent\",\"params\":{\"pane_id\":\"pane\\\"x\"," ++
            "\"source\":\"custom:fx\",\"agent\":\"hx\",\"state\":\"blocked\"}}\n",
        out.written(),
    );
}
...
test "clear_agent_authority removes hx from the pane" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
...
test "report_agent_session serializes session identity" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"3\",\"method\":\"pane.report_agent_session\",\"params\":{\"pane_id\":\"w1:p1\"," ++
            "\"source\":\"custom:fx\",\"agent\":\"hx\",\"agent_session_id\":\"session-42\"}}\n",
        out.written(),
    );
}
