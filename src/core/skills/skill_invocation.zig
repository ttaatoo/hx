...
test "skill invocation preserves hard allocation failures" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.hx/skills/workflow");
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.hx/skills/workflow/SKILL.md", .{});
        defer file.close(io_mod.getIo());
    ...
    const skills_dir = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home/.hx/skills");
...
test "skill invocation loads installed skill content" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/workspace");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.hx/skills/workflow/assets");
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.hx/skills/workflow/SKILL.md", .{});
        defer file.close(io_mod.getIo());
        try file.writeStreamingAll(io_mod.getIo(), "---\nname: workflow\ndescription: workflow helper\n---\n\nuse the workflow skill\n");
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.hx/skills/workflow/assets/data.txt", .{});
        defer file.close(io_mod.getIo());
...
test "stale skill catalogs reject mutated candidates before loading" {
    ...
    inline for (.{ "deleted", "invalid", "renamed" }) |mutation| {
        ...
        if (std.mem.eql(u8, mutation, "deleted")) {
            try tmp.dir.deleteFile(io_mod.getIo(), "home/.hx/skills/workflow/SKILL.md");
        } else {
            var file = try tmp.dir.createFile(
                io_mod.getIo(),
                "home/.hx/skills/workflow/SKILL.md",
                .{ .truncate = true },
            );
            defer file.close(io_mod.getIo());
...
const CandidatePathReplacementHook = struct {
    ...
    fn check(raw: *anyopaque) !void {
        ...
        try self.dir.rename(
            "home/.hx/skills/workflow",
            self.dir.*,
            "home/.hx/skills/workflow-original",
            io_mod.getIo(),
        );
        try self.dir.rename(
...
test "large skills stay discoverable and explicit overrides can continue past default file bounds" {
    ...
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "home/workspace");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.hx/skills/large");
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.hx/skills/large/SKILL.md", .{});
        defer file.close(io_mod.getIo());
...
test "explicit invocation preserves configured skill content when discovery is incomplete" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/workspace");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.hx/skills/large");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.hx/skills/malformed");
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.hx/skills/malformed/SKILL.md", .{});
        defer file.close(io_mod.getIo());
        try file.writeStreamingAll(io_mod.getIo(), "---\ndescription: missing name\n---\nMALFORMED BODY");
    }
...
test "skill resources continue on line-safe UTF-8 boundaries and reject unsafe inputs" {
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.hx/skills/workflow/assets/binary.bin", .{});
        defer file.close(io_mod.getIo());
    ...
    inline for (.{
        ...
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.hx/skills/workflow/assets/" ++ fixture[0], .{});
        defer file.close(io_mod.getIo());
        try file.writeStreamingAll(io_mod.getIo(), fixture[1]);
    }
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.hx/skills/workflow/assets/cross-chunk.txt", .{});
        defer file.close(io_mod.getIo());
