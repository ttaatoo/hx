...
test "skill tool loads the exact advertised duplicate and rejects ambiguous or untrusted locations" {
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.hx/skills/workflow/assets/b-only.txt", .{});
        defer file.close(io_mod.getIo());
    ...
    const managed_skill = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home/.hx/skills/workflow");
