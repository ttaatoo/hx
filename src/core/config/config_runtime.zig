...
test "discoverPathsFromHome returns home-backed and workspace paths" {
    ...
    try std.testing.expectEqualStrings("/Users/tester/.hx/settings.json", paths.user_settings.?);
    try std.testing.expectEqualStrings("/tmp/workspace/.fx.json", paths.workspace_settings);
    try std.testing.expectEqualStrings("/Users/tester/.hx", paths.home_fx_dir.?);
    try std.testing.expectEqualStrings("/Users/tester/.hx/sessions", paths.sessions_dir.?);
...
test "merged settings rejects writable user policy files" {
    ...
    var root_dir = try tmp.dir.openDir(io_mod.getIo(), "home/.hx", .{});
...
test "loadMergedSettings merges project defaults before profile layers" {
    ...
    try writeFixtureFile(tmp.dir, "home/.hx/settings.json", user_settings);
...
test "invalid permission mode returns InvalidPermissionMode" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.hx");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeFixtureFile(tmp.dir, "home/.hx/settings.json", "{\"permission_mode\":\"danger\"}");
...
test "nested permission config preserves JSON object order" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.hx");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeFixtureFile(tmp.dir, "home/.hx/settings.json", "{\"permission\":{\"*\":\"ask\",\"bash\":{\"git *\":\"allow\",\"git push *\":\"deny\"},\"edit\":\"deny\"}}");
...
test "userSettingsPath follows absent and present HOME" {
...

    {
        ...
        try std.testing.expectEqualStrings("/Users/tester/.hx/settings.json", path);
    }
}
...
test "HOME test helper remains stable across absent A and B states" {
...

    {
        ...
        const path = (try userSettingsPath(std.testing.allocator)).?;
        defer std.testing.allocator.free(path);
        try std.testing.expectEqualStrings("/home/a/.hx/settings.json", path);
    }

    {
        ...
        const path = (try userSettingsPath(std.testing.allocator)).?;
        defer std.testing.allocator.free(path);
        try std.testing.expectEqualStrings("/home/b/.hx/settings.json", path);
    }
}
...
test "detailed settings preserve model precedence and source" {
    ...
    try writeFixtureFile(tmp.dir, "home/.hx/settings.json", user_fixture);
...
test "detailed settings report non-empty process model override as winning source" {
    ...
    const workspace_root = try io_mod.dirRealpathAlloc(std.testing.allocator, tmp.dir, "workspace");
    defer std.testing.allocator.free(workspace_root);
    try writeFixtureFile(tmp.dir, "home/.hx/settings.json", "{\"model\":\"user/model\"}\n");
...
test "invalid user model emits typed diagnostic and project model is ignored" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.hx");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeFixtureFile(tmp.dir, "home/.hx/settings.json", "{\"model\":\" invalid/model \"}\n");
    try writeFixtureFile(tmp.dir, "workspace/.fx.json", "{\"model\":\"project/model\"}\n");
    const home_root = try io_mod.dirRealpathAlloc(std.testing.allocator, tmp.dir, "home");
    defer std.testing.allocator.free(home_root);
    const workspace_root = try io_mod.dirRealpathAlloc(std.testing.allocator, tmp.dir, "workspace");
...
test "detailed settings report unsafe user permissions distinctly" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.hx");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeFixtureFile(tmp.dir, "home/.hx/settings.json", "{\"permission_mode\":\"auto\"}\n");
    var root_dir = try tmp.dir.openDir(io_mod.getIo(), "home/.hx", .{});
    defer root_dir.close(io_mod.getIo());
    var file = try root_dir.openFile(io_mod.getIo(), "settings.json", .{ .mode = .read_write });
    file.setPermissions(io_mod.getIo(), std.Io.File.Permissions.fromMode(0o666)) catch {
...
test "invalid user settings report newest valid manual recovery backup" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeFixtureFile(tmp.dir, "home/.hx/settings.json", "{broken");
    try writeFixtureFile(
        tmp.dir,
        "home/.hx/backups/settings.json.backup.100-0000000000000001-00000000000000000000000000000000",
        "{\"model\":\"backup/model\"}\n",
    );
    const home_root = try io_mod.dirRealpathAlloc(std.testing.allocator, tmp.dir, "home");
    defer std.testing.allocator.free(home_root);
...
test "additional directories load only from the current profile workspace" {
    ...
    try writeFixtureFile(tmp.dir, "home/.hx/settings.json", settings_fixture);
