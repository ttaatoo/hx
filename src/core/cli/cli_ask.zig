...
fn runAskChild(
    ...
    return subagent_agent_adapter.run(.{
        ...
        .provider_routes = .{
            ...
            .direct = .{
                .agent_stream_provider = @import("../../gateway/direct_provider.zig").agent_stream_provider,
...
/// Resume selector parsed from hx ask --resume.
...
const AskContext = struct {
    ...
    fn initializeSessionStores(self: *AskContext) !void {
        var store = session_store.Store.init(self.alloc, self.workspace_root) catch |err| {
            ...
            try self.writeStderr("hx ask: warning: session persistence unavailable; error=");
...
fn writeAskUsage(deps: RunDeps, usage: []const u8) !void {
    try deps.write_stderr(deps.stderr_ctx, "usage: hx ");
...
fn runWithDeps(alloc: Allocator, args: []const [:0]const u8, cfg: Config, deps: RunDeps) !u8 {
    ...
    var options = parseOptionsWithStdin(alloc, args, deps.stdin_source) catch |err| switch (err) {
        error.MissingPrompt => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "hx ask: missing prompt\n");
        ...
        error.NoSaveResumeConflict => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "hx ask: --no-save cannot be used with --resume or --resume-id\n");
            try writeAskUsage(deps, cfg.command_usage);
            return 1;
        },
        error.PromptResourceLimitExceeded => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "hx ask: prompt exceeds the local input safety limit\n");
            return 1;
        },
        error.PromptInputReadFailed => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "hx ask: failed to read prompt from stdin\n");
            return 1;
        },
        ...
        error.InvalidPromptText => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "hx ask: prompt must be valid UTF-8 and contain no NUL bytes\n");
            return 1;
        },
        else => return err,
    };
    ...
    const result = runPromptInternal(alloc, options.prompt, options.permission_override, effective_cfg, .{
        ...
        if (err == error.OneOffSessionNotResumable and !options.json_output) {
            try deps.write_stderr(
                deps.stderr_ctx,
                "hx ask: one-off child sessions cannot accept additional prompts; create a persistent child to continue the conversation\n",
            );
            return 1;
        }
    ...
    for (options.image_paths.items) |image_path| {
        const attachment = image_attachments.loadUserImageAttachment(
            ...
            if (options.json_output) {
                ...
                try message.writer.print("hx ask: failed to attach image \"{s}\": {s}\n", .{ image_path, reason });
...
fn missingCredentialResult(
    ...
    try options.deps.write_stderr(options.deps.stderr_ctx, "hx ask: ");
...
fn finishCliPermissionOutcome(
    ...
    switch (outcome.requirement orelse .approval_required) {
        .configured_rule => try writeBlockedActionGuidance(
            ctx,
            "hx ask: permission required by configured rule",
            label,
            "noninteractive_permission_prompt_unavailable",
            "hx ask: rerun in the interactive shell to approve this action, or add a narrow matching permission rule before retrying\n",
        ),
        .approval_required, .sandbox_widening => try writeBlockedActionGuidance(
            ctx,
            "hx ask: permission required for tool execution in noninteractive mode",
            label,
            "noninteractive_permission_prompt_unavailable",
            if (permission_mode == .auto)
                "hx ask: human approval is required for this action; use the interactive shell to approve it, or add a narrow matching permission rule\n"
            else
                "hx ask: rerun with --auto to review this exact action automatically, or use the interactive shell to approve it\n",
        ),
    }
...
fn emitAskNotificationBell(raw: *anyopaque) void {
    ...
    ctx.writeStderr("\x07") catch |err| {
        debug_trace.logf(
            "notifications",
            "hx ask terminal bell write failed err={s}",
            .{@errorName(err)},
        );
    };
}
    ...
    switch (attempt) {
        ...
        .failure => |failure| {
            ...
            try message.writer.print(
                "hx ask: failed to save YOLO acknowledgment: {s}\n",
...
fn promptRealPermissionApproval(
    ...
    try write_stderr(stderr_ctx, "hx wants to run:\n  ");
...
test "hx ask deps validate malformed registered calls" {
    const alloc = std.testing.allocator;
...
test "hx ask ChatGPT route disables Gateway-backed auxiliary providers" {
    const alloc = std.testing.allocator;
    var stdout_capture = TestCapture{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture = TestCapture{};
...
test "hx ask finalization fails every failed turn" {
...
test "hx ask deps reject malformed native web_search calls" {
    const alloc = std.testing.allocator;
    var stdout_capture = TestCapture{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture = TestCapture{};
...
test "headless yolo warning precedes startup configuration diagnostics" {
    ...
    try std.testing.expect(std.mem.startsWith(
        ...
        stderr_capture.bytes.items,
        permissions.yolo_warning_text ++ "\n" ++
            "hx ask: config user: malformed_settings\n",
    ));
}
...
test "stdin read failure has distinct text and JSON output contracts" {
    ...
    try std.testing.expectEqualStrings(
        "hx ask: failed to read prompt from stdin\n",
        stderr_capture.bytes.items,
    );
...
test "hx ask default user commands require configured authority or review" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "hx ask automatic review observes worker cancellation" {
...
test "hx ask auto mode applies automatic allow and ask without a prompt" {
...
test "hx ask terminal permission prompt approves and denies run_command" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "hx ask permission attention fires once after a prompt is published" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "hx ask bell writes only to TTY stderr" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "hx ask registers notification handlers only when configured" {
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(std.testing.allocator);
    var stderr_capture: TestCapture = .{};
    defer stderr_capture.deinit(std.testing.allocator);
...
test "hx ask captured and quiet permission paths bypass terminal prompt" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "hx ask permission prompt policy requires explicit captured-mode opt in and TTY stdin" {
    const Case = struct {
        output_mode: OutputMode,
        prompt_permissions: bool,
        stdin_is_tty: bool,
...
test "hx ask captured permission prompt opt in uses the existing prompter" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "hx ask terminal permission prompt propagates prompt hook errors" {
...
test "hx ask prepared file mutation callback preserves terminal permission prompt" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
...
test "hx ask headless sandbox widening keeps exhausted recovery model-visible" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "hx ask auto mode uses automatic allow for external prepared file mutation" {
    const FakeClassifier = struct {
        calls: usize = 0,
        root_text: []const u8 = "",
...
test "hx ask preserves CLI headless blocker diagnostics" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    ...
    const expected_configured_stderr = try std.fmt.allocPrint(
        arena,
        "hx ask: permission required by configured rule\n" ++
            "hx ask: blocked action: {s}\n" ++
            "hx ask: reason=noninteractive_permission_prompt_unavailable\n" ++
            "hx ask: rerun in the interactive shell to approve this action, or add a narrow matching permission rule before retrying\n",
        .{configured_label},
    );
    try std.testing.expectEqualStrings("", stdout_capture.bytes.items);
...
test "hx ask ordinary multi-target admission preserves deny precedence without diagnostics" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
...
test "hx ask renders one-off resume denial in text and JSON modes" {
    const alloc = std.testing.allocator;
    const cases = [_]struct {
...
test "saved ask classifies unsafe store failure by request mode" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    var blocker = try tmp.dir.createFile(io_mod.getIo(), "home/.hx", .{});
    ...
    try std.testing.expectEqualStrings(
        "hx ask: warning: session persistence unavailable; error=SessionPathUnsafe; continuing without saving\n",
        stderr_capture.bytes.items,
    );
...
test "hx ask JSON captures HTTP 413 prompt-too-long blocker" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expect(std.mem.find(u8, stderr_capture.bytes.items, "hx ask: HTTP 413: provider payload rejected") != null);
...
test "hx ask formats restricted-provider HTTP errors without raw JSON" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expect(std.mem.find(u8, stderr_capture.bytes.items, "hx ask: API access denied · HTTP 403 · Provider: wafer") != null);
...
test "hx ask text and JSON share the selected auth failure facts" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expectEqualStrings(
        "hx ask: SuperGrok subscription authentication failed · HTTP 401\n",
        stderr_capture.bytes.items,
    );
    var parsed = try std.json.parseFromSlice(std.json.Value, alloc, stdout_capture.bytes.items, .{});
    defer parsed.deinit();
    ...
    try std.testing.expectEqualStrings("SuperGrok subscription", auth_failure.get("source").?.string);
...
test "hx ask JSON records permission-denied tool calls as error status" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
...
test "hx ask JSON permission-denied capture is best effort under allocation failure" {
    var failing = std.testing.FailingAllocator.init(
        std.testing.allocator,
        .{ .fail_index = 0 },
    );
...
test "hx ask defers cancelled sandbox retry capture to the common projection" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "hx ask JSON captures parallel tool results without corrupting records" {
    const alloc = std.heap.c_allocator;
...
test "hx ask JSON capture cleans up every allocation failure" {
...
test "hx ask JSON records ask_user_question text for matching assertions" {
    const alloc = std.testing.allocator;
    const records = try alloc.alloc(ToolCallRecord, 1);
    records[0] = .{
...
test "hx ask JSON clips ask_user_question text at a UTF-8 boundary" {
    const alloc = std.testing.allocator;
...
test "json run with missing API key prints diagnostic then final object" {
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expectEqualStrings("hx ask: " ++ credentials.missing_credential_message ++ "\n", stderr_capture.bytes.items);
    try std.testing.expectEqualStrings(
...
test "hx ask emits one discovery warning when catalog truncation and a skill read report it" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "hx ask carries resolved auto mode and initial registry context into the queued prompt" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "default hx ask passes canonical image paths as initial context targets" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    {
...
test "default hx ask omits disabled registry context without gathering" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "default hx ask preserves project context gathering error mappings" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "hx ask JSON recovery keeps stdout structured and reports progress on stderr" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "hx ask JSON preserves partial output on prompt failure" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "hx ask raw output still propagates prompt failure" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
