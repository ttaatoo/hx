...
const product = @import("../shared/product.zig");
...
pub fn renderTopLevelHelpWithStyle(alloc: Allocator, registry: TopLevelRegistry, columns: usize, version: []const u8, style: HelpStyle) ![]u8 {
    ...
    try writeStyled(&out.writer, style, .brand, "hx");
    ...
    try writeWrappedStyledLine(&out.writer, "  ", "  ", product.name ++ " [flags]", width, style, .syntax);
    try writeWrappedStyledLine(&out.writer, "  ", "  ", product.name ++ " <command> [...flags] [...args]", width, style, .syntax);
...
pub fn renderTopLevelCommandHelp(alloc: Allocator, registry: TopLevelRegistry, kind: TopLevelKind) ![]u8 {
    ...
    try out.writer.writeAll(product.name);
...
test "rendered top-level help is a complete CLI navigation page" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, text, "hx v9.8.7\nFast, native coding agent for the terminal."));
    try std.testing.expect(std.mem.find(u8, text, "hx starts an interactive session by default.") != null);
    try std.testing.expect(std.mem.find(u8, text, "hx <command> [...flags] [...args]") != null);
    ...
    try std.testing.expect(std.mem.find(u8, text, "hx ask \"Explain the changes in this repository\"") != null);
    try std.testing.expect(std.mem.find(u8, text, "hx session resume last") != null);
    try std.testing.expect(std.mem.find(u8, text, "session resume [last|id]") != null);
    try std.testing.expect(std.mem.find(u8, text, "hx status --json") != null);
    try std.testing.expect(std.mem.find(u8, text, "Run `/help` inside an interactive session for slash commands.") != null);
    try std.testing.expect(std.mem.find(u8, text, "Learn more about hx:  https://github.com/ttaatoo/hx") != null);
    try std.testing.expect(std.mem.find(u8, text, "\nReport a problem:     run `/feedback` inside hx\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "\n\n\nRun `hx <command> --help`") == null);
...
test "terminal top-level help adds styling without changing visible content" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, terminal, "\x1b[1mhx\x1b[0m"));
    ...
    try std.testing.expect(std.mem.find(u8, terminal, "\x1b[4mhttps://github.com/ttaatoo/hx\x1b[0m") != null);
    try std.testing.expect(std.mem.find(u8, terminal, "\x1b[39mrun `/feedback` inside hx\x1b[0m") != null);
...
test "top-level help renders flags as compact aligned rows" {
    ...
    try std.testing.expect(std.mem.find(u8, wide, "Print the hx version and exit\n\nExamples:") != null);
...
test "per-command help renders header usage options and details" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "hx permissions\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  hx permissions [--json]") != null);
...
test "per-command help preserves long resume usage outside top-level index" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  hx session resume [last|<id>] [--record] | session resume --id <id> [--record] | --resume [last|<id>] [--record] | resume [last|<id>] [--record] | resume --id <id> [--record] | --resume-last | --continue | -c | -r | --resume-<id>") != null);
    try std.testing.expect(std.mem.find(u8, text, "Options:") != null);
...
test "ACP help documents accepted options" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "hx acp\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  hx acp [--model <id>] [--log-file <path>]") != null);
...
test "slash completion descriptions follow completion matches" {
    ...
    try std.testing.expectEqualStrings("open the hx feedback form", nthSlashCompletionDescription(testSlashRegistry(), "/fee", 0).?);
