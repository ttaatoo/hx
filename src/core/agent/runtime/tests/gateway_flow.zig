...
test "processQueuedPrompt rejects text-only images without native vision" {
    const alloc = std.testing.allocator;
    ...
    for ([_]model_provider.ProviderId{ .xai, .anthropic }) |provider| {
...
test "processQueuedPrompt routes images natively only when vision and file input are both supported" {
    ...
    for (cases) |entry| {
        ...
        if (entry.expect_native) {
            ...
            try std.testing.expect(std.mem.find(u8, gateway.request_bodies.items[0], image_path) == null);
        } else {
            try std.testing.expectError(
                error.CodexNativeImageUnavailable,
                runFakePrompt(&gateway, &hooks, fixture.config(), job),
...
test "processQueuedPrompt pauses when uncertain tool reconciliation returns another tool" {
    const alloc = std.testing.allocator;
    const starts = [_]ToolCall{toolCall("provider_search", "perplexity_search", "{}")};
    const repeated = [_]ToolCall{toolCall("provider_search_repeat", "perplexity_search", "{}")};
    const completions = [_]FakeCompletion{
        .{
...
test "processQueuedPrompt reconciles ReadFailed after provider-executed tool start" {
    const alloc = std.testing.allocator;
    const starts = [_]ToolCall{toolCall("call_search", "perplexity_search", "{}")};
    const completions = [_]FakeCompletion{
        .{
            .streamed_tool_starts = &starts,
            .stream_error_after_tool_starts = error.ReadFailed,
...
test "processQueuedPrompt refreshes hx login credential before gateway request" {
    const alloc = std.testing.allocator;
    const completions = [_]FakeCompletion{.{ .content = "Done." }};
    var gateway = FakeGateway.init(alloc, &completions);
    defer gateway.deinit();
...
test "processQueuedPrompt refreshes and retries once after hx login 401" {
    const alloc = std.testing.allocator;
    const completions = [_]FakeCompletion{
        .{
            .status = .unauthorized,
...
test "processQueuedPrompt does not retry a second hx login 401" {
    const alloc = std.testing.allocator;
    const completions = [_]FakeCompletion{
        .{
            .status = .unauthorized,
...
test "processQueuedPrompt keeps the selected hx login credential when forced refresh is unavailable" {
    const alloc = std.testing.allocator;
    const completions = [_]FakeCompletion{
        .{
            .status = .unauthorized,
