...
fn onboardingProjectedRowIndex(view: auth_runtime.PickerView, row_index: u16, row_count: u16) u16 {
    ...
    const priority = [_]u16{ selected_row, other_row, 10, 11, 15, 7, 13, 5, 0, 2, 3, 6, 12, 14, 1, 4, 16, 17 };
...
test "mixed slash completion uses skill relevance order" {
    const skills = [_]skill_runtime.Skill{
        .{
            ...
            .path = "/tmp/.hx/skills/metadata-first",
            .source = .global_fx,
        },
        .{
...
test "auth onboarding composes the welcome copy and setup choices" {
    ...
    const view = auth_runtime.PickerView{
        ...
        .selected_choice = .{ .action = .grok_login },
        .active_source = null,
        .include_skip = true,
    };
    ...
    try std.testing.expect(std.mem.find(u8, selected_row.items, "› Sign in with SuperGrok") != null);
...
test "auth picker composes only detected credential sources" {
    ...
    var grok = try composeAuthPickerRow(alloc, view, 1, row_count, 80);
    defer grok.deinit(alloc);
    try std.testing.expect(std.mem.find(u8, grok.items, "Sign in with SuperGrok") != null);
