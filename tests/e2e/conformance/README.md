...
built hx binary. It is pinned to:
...
`package-lock.json` and installs the pinned official runner. Bun executes hx's
...
starting scenarios when the freshly built `zig-out/bin/hx` is missing.
...
`./zig-out/bin/hx` with dummy `grok-auth.json` and `--yolo`. It does not
