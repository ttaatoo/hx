...
describe("agent quality A/B harness helpers", () => {
  ...
  test("rejects bare fx and relative binary paths", () => {
    ...
    expect(() => requireAbsoluteExecutableBinary("./zig-out/bin/hx", "candidate")).toThrow(/absolute path/);
  });

  test("redacts credential-looking values", () => {
    expect(redactSensitiveValue("ANTHROPIC_API_KEY", "secret-value")).toBe("[redacted]");
